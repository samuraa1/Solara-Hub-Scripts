...
if not game.CoreGui:FindFirstChild("ScreenGui") then
    ...
    local drawings = loadstring(game:HttpGet("https://raw.githubusercontent.com/samuraa1/Solara-Hub-Scripts/refs/heads/main/Drawings.lua", true))()
    ...
    for indexDrawing, drawing in ipairs(drawings) do
            ...
            if imageButton.Active then
                if drawingFinished then
                    if game.Players.LocalPlayer.Character.Humanoid.Sit then
                        ...
                        game.StarterGui:SetCore("SendNotification", {Title = "DRAWING STARTED", Text = drawing.drawingName, Icon = drawing.drawingAsset, Duration = 3})
                        local userCanvas = game.Players.LocalPlayer.PlayerGui.MainGui.PaintFrame.GridHolder.Grid
                        for indexPixel, pixel in ipairs(drawing.drawingPixels) do
                            if game.Players.LocalPlayer.Character.Humanoid.Sit then
                                ...
                                userCanvas[indexPixel].BackgroundColor3 = Color3.fromRGB(pixel.R, pixel.G, pixel.B)
                                if currentPixel == 1024 then
                                    game.StarterGui:SetCore("SendNotification", {Title = "DRAWING DONE", Text = drawing.drawingName, Icon = drawing.drawingAsset, Duration = 3})
                                    currentPixel = 0
                        ...
                        game.StarterGui:SetCore("SendNotification", {Title = "SIT DOWN PLEASE", Text = "Sit down to draw", Duration = 3})
                    end
                else
                    game.StarterGui:SetCore("SendNotification", {Title = "WAIT CURRENT DRAWING", Text = "Wait for the current drawing", Duration = 3})
                end
            end
        end)

        imageAsset.MouseEnter:Connect(function()
        ...
        imageAsset.MouseLeave:Connect(function()
            imageAsset.ImageTransparency = 0.5
            imageName.TextTransparency = 0.5
            imageName.BackgroundTransparency = 0.7
            imageButton.BackgroundColor3 = Color3.fromHex("000000")
            ...
            if drawingPaused then
                ...
                for indexPixel = currentPixel, #currentDrawing.drawingPixels do
                    if game.Players.LocalPlayer.Character.Humanoid.Sit then
                        currentPixel = indexPixel
                        drawingPixelsValue.Text = currentPixel .. " of 1024"
                        userCanvas[indexPixel].BackgroundColor3 = Color3.fromRGB(currentDrawing.drawingPixels[indexPixel].R, currentDrawing.drawingPixels[indexPixel].G, currentDrawing.drawingPixels[indexPixel].B)
                        if currentPixel == 1024 then
                            game.StarterGui:SetCore("SendNotification", {Title = "DRAWING DONE", Text = currentDrawing.drawingName, Icon = currentDrawing.drawingAsset, Duration = 3})
                            currentDrawing = {}
                            currentPixel = 0
                            drawingFinished = true
                            drawingPaused = false
